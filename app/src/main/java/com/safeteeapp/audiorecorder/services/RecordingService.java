package com.safeteeapp.audiorecorder.services;
...
import com.safeteeapp.audiorecorder.models.RecordingsDatabase;
import com.safeteeapp.audiorecorder.helpers.WavConverter;
...
public class RecordingService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if ("com.safetee.soundrecorder.START_RECORDING".equals(intent.getAction())) {
			startRecording();
		} else if ("com.safetee.soundrecorder.STOP_RECORDING".equals(intent.getAction())) {
			stopRecording();
		}
		return START_STICKY;
	}
	...
	private void startTimer() {
		...
		mIncrementTimerTask = new TimerTask() {
			...
			public void run() {
				...
				// it is really not that descret to show the recording
				// a lady from survey monkey that cali introduced us to at the hub today '12/19/16' made valid point on this
	...
	private Notification createNotification() {
		...
		builder.setContentIntent(PendingIntent.getActivities(getApplicationContext(), 0,
				new Intent[] { new Intent(getApplicationContext(), MainActivity2.class) }, 0));

		Intent stopIntent = new Intent(getApplicationContext(), RecordingService.class);
		stopIntent.setAction("com.safetee.soundrecorder.STOP_RECORDING");
	...
	public void stopRecording() {
		...
		// user setting if to auto upload or not
		if (session.getAutoupload()){
			// go auto upload record
	...
	public void uploadRecording(File newFile) {
		...
		try {
			String uploadId =
					...
					.addParameter("share", "false")
							// les just disable upload notification
							// but we will go ahead and send the record to server if auto upload is enabled in record setting
							// for record safe keeping incase...
							// since we will be pulling records for user locally, what if they lost their phone or it got destroyed right after the thing happen???
							// even if it doesnt upload maybe cox of network issue, when requesting for support, safetee algorithm will check if record has been uploaded before or not
						    // if not it will upload the record.
	...
	private File generateFile() {
		...
		// "." is needed to have records folder hidden
		filename += "/data/data/" + getPackageName() +"/.SafeteeRecords";
